...
import {
  onBoardingHandleNext,
  onBoardingHandlePrevious,
} from "@/feature/reducers/userOnboarding";
import {
  getAllSalesforceSkills,
  SkillsService,
} from "@/lib/service/portfolio.service";
...
const CheckboxItem = ({
  ...
  if (checkedItem?.bgColor) {
    return (
      <div
        className={`${checkedItem.bgColor} border ${checkedItem.borderColor} p-2 pl-4 rounded-xl relative z-10 whitespace-nowrap min-w-max`}
      >
        <div className="inline-flex gap-2 items-center">
          <img
            className="w-auto h-6"
            src={checkedItem.imageSrc}
            alt={checkedItem.text}
          />
        ...
        <div className="text-xs text-black">Skill Level: {skill_level}</div>
      </div>
    );
  }
};
...
const Skills = () => {
  ...
  const getSkillsDetails = async () => {
    try {
      ...
      const { results: assignedSkills } =
        await SkillsService.getAllAssignedSkills(contactSfid);
      console.log("assignedSkills::", assignedSkills);

      const assignedSkillsIds = assignedSkills.map(
        (skill: any) => skill.skill.sfid
      );

      setInitialItems(allSkills);
      setInitialCheckedItems(assignedSkills);
      setItems(
        allSkills.filter((item: any) => !assignedSkillsIds.includes(item.sfid))
      );
      setFilteredItems(
        allSkills.filter((item: any) => !assignedSkillsIds.includes(item.sfid))
      );
  ...
  const deleteSkills = async (sfid: string) => {
    try {
      setLoadingUI(true);
      const id = initialCheckedItems.find(
        (item: any) => item.skill.sfid === sfid
      )?.sfid;
    ...
    if (isChecked) {
      ...
      if (uncheckedItem) {
        ...
        if (response) {
          ...
          toast.custom((t) => (
            <SuccessfulToast
              t={t}
              message={"Salesforce Skill deleted Successfully"}
            />
          ));
        } else {
          toast.custom((t) => (
      ...
      if (response) {
        ...
        toast.custom((t) => (
          <SuccessfulToast
            t={t}
            message={"Salesforce Skill added Successfully"}
          />
        ));
      } else {
        toast.custom((t) => (
  ...
  return (
    <>
      <div className="bg-[url(/noRecordBG2.png)] bg-contain bg-no-repeat bg-bottom bg-white rounded-3xl border border-gray-300 overflow-clip w-full h-full relative px-5 lg:px-10">
            ...
            {showSuggestions && !loadingUI && (
              <div className="absolute bg-white border-2 left-0 rounded-xl shadow-sm w-full lg:w-1/2 mt-1 max-h-48 overflow-y-auto z-20">
                {filteredItems.length > 0 ? (
                  filteredItems
                    .sort((a, b) => {
                      const nameA = a?.name || a?.skill?.name || "";
                      const nameB = b?.name || b?.skill?.name || "";
                      return nameA.localeCompare(nameB);
                    })
                    .map((item) => (
                      <div
                        className={
                          showSkillLevel === item.sfid ? "bg-gray-50" : ""
                        }
                      >
                        <div
                          key={item.sfid}
                          className={`flex flex-row justify-between px-4 items-center gap-2 p-3 border border-b-0 cursor-pointer ${
                  ...
                  <div className="text-md text-gray-400 p-4">
                    No matching skills
                  </div>
                )}
              </div>
        ...
        <h2 className="text-sm uppercase font-semibold z-10">
          Selected Skills
        </h2>
        <div className="pt-4 flex flex-row gap-4 flex-nowrap lg:flex-wrap z-40 overflow-x-auto w-full appearance-none">
          {loading ? (
            <>
              <div
                className={`animate-pulse inline-flex w-1/6 h-8 gap-2 items-center bg-gray-200 border p-2 pl-4 pr-4 rounded-full`}
              />
              <div
                className={`animate-pulse inline-flex w-1/6 h-8 gap-2 items-center bg-gray-200 border p-2 pl-4 pr-4 rounded-full`}
              />
                ...
                checkedItems.map((item) => (
                  <CheckboxItem
                    ...
                    checked={checkedItems.some(
                      (checkedItem) => checkedItem.sfid === item.sfid
                    )}
                    onChange={() =>
                      handleCheckboxChange(item?.skill?.sfid || item?.sfid)
                    }
