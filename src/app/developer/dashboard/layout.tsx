...
function DeveloperDashboardLayout({
    ...
    return (
        <>
            <title>Developer | DX Digital</title>
            <section className={`${geistSans.variable} ${geistMono.variable} h-screen bg-gray-100 text-black`}>
                <div className="grid grid-cols-12 grid-rows-12 gap-5 min-h-screen max-h-screen p-4">
                    ...
                    <main className={`${isUserOnboarded ? `col-span-12 col-start-1 ${toggleSideBar ? "lg:col-span-10 lg:col-start-3" : "lg:col-span-11 lg:col-start-2"} ` : "col-span-12"} row-span-10 row-start-3 w-full h-full overflow-y-scroll no-scrollbar`}>
                        {children}
                    </main>
                </div>
            </section>
